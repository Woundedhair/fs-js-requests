...
function changeButtonColor () {
  ...
  //sayHelloButton.classList.add('hello-style');
...
sayHelloButton.addEventListener('mouseover', revertButtonColor);
...
const repeatMyParam = () => {
  axios.get('http://localhost:3000/repeat/Akin').then((response) => {
  console.log(response.data);
...
function getRequest(name) {
  axios.get(`http://localhost:3000/query-test?name=${name}`)
  .then(response => {
    console.log(response.data)
})
}
